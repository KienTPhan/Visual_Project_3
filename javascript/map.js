...
function draw_new_map(damage_area) {
  // create a new map svg using damage_area to pick the correct div
  var map_svg = d3.select("#map-div-"+ damage_area)
                .append("svg")
                .attr('id','map-svg-'+ damage_area)
                .attr('height', mapHeight + mapMargin.top + mapMargin.bottom)
                .attr('width', mapWidth + mapMargin.left + mapMargin.right)
                .append('g')
                .attr('transform','translate(' + mapMargin.left + ',' + mapMargin.top + ')');

  // make scale legend for damage_area map 
  var legend = map_svg.append("g")
    .attr('id','legend_group-'+damage_area)
    .attr("font-family", "sans-serif")
    .attr("font-size", 15)
    .attr("text-anchor", "end")
    .selectAll("g")
  ...
  // Temp quick fix that might stay: To chang legend y axis
  d3.select('#legend_group'+damage_area)
  .attr("transform", function(d, i) { return "translate(50,0)"; });
...
// new update_map using 
function updateMap_damage_area(newData,damage_area) {
  ...
          .rollup(function(d) {
    ...
    d3.json("/data/StHimark.geojson").then(function(geojson) {
      ...
      for(var i = 0; i < aggregated_data.length; i++){
        ...
        var neighbor_id = aggregated_data[i].key;
        //grab data value 
        var damage_value = eval("aggregated_data[i].value.mean_" + damage_area);  //TODO: need to make it work for all categories
        //find the corresponding neighbor inside the GeoJSON
        for(var n = 0; n < geojson.features.length; n++){
          // properties name gets the neighbor's name
          var jsonNeighbor_id = geojson.features[n].properties.Id;
    ...
    // select all paths that makes up the damage_area map
    var neighbors_paths =  d3.select('#map-svg-'+ damage_area).select('g').selectAll(".neighbors_"+damage_area);
    ...
    enter
        .append('path')
        .attr('class',".neighbors_"+ damage_area)
        .attr( 'd', map_path)
        .attr( 'fill', function(d){
          // if data given is blank
          if (d.properties.damage_value === null){
